...
function Idea() {
  return (
    <section className="idea-layout"><div className="slide-copy"><SectionLabel number="04">Основная идея</SectionLabel><h2>Приложение отвечает<br />на три вопроса</h2><p>Главный экран начинает не с длинной витрины, а с регулярной задачи пользователя.</p><LinkButton href={proto('/mode')}>Проверить первый шаг</LinkButton></div><div className="question-stack"><article><span>Где и когда?</span><strong>Доставка или конкретная кулинария — до наполнения корзины.</strong><PackageCheck /></article><article><span>Что доступно?</span><strong>Меню и интервалы для выбранной точки.</strong><Store /></article><article><span>Как повторить?</span><strong>Прошлый заказ проходит новую проверку цены и наличия.</strong><RefreshCw /></article></div></section>
  )
}
...
function Contact() {
  return (
    <section className="contact-layout"><div className="contact-orbit"><Smartphone /><span>Рабочий прототип<br />уже можно пройти</span></div><div className="contact-copy"><SectionLabel number="12">Следующий шаг</SectionLabel><h2>Покажем прототип<br /><em>лично в Краснодаре</em></h2><p>Предлагаем вместе пройти три приоритетных сценария, выбрать границы пилота и определить, какие данные нужны для сравнения.</p><div className="contact-links"><a href="mailto:hello@eh.works">hello@eh.works <ArrowRight /></a><a href="https://eh.works" target="_blank" rel="noreferrer">eh.works <ArrowRight /></a></div><LinkButton href={proto('/')}>Открыть прототип</LinkButton><small>ООО «ЭРГОХАВЭН» · аккредитованная ИТ-компания</small></div></section>
  )
}
