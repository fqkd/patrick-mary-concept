...
function CakeScenario() {
  return (
    <section className="scenario-layout"><div className="slide-copy"><SectionLabel number="07">Сценарий № 3</SectionLabel><h2>Заявка на<br />индивидуальный торт</h2><p>Пользователь указывает повод, количество гостей, оформление, дату и телефон. Затем проверяет и сохраняет заявку.</p><div className="cake-steps"><span>1<small>Повод</small></span><span>2<small>Гости</small></span><span>3<small>Оформление</small></span><span>4<small>Дата</small></span><span>5<small>Проверка</small></span></div><LinkButton href={proto('/cake?step=occasion')}>Оставить заявку</LinkButton></div><div className="cake-showcase"><Phone src="assets/case/cake.jpg" alt="Выбор оформления индивидуального торта" /><div className="cake-card"><CakeSlice /><span>После заявки сотрудник уточнит детали и стоимость</span></div></div></section>
  )
}

function BusinessFit() {
  return (
    <section className="standard-layout fit-slide"><div className="slide-copy"><SectionLabel number="08">Сценарии покупки</SectionLabel><h2>Для обычного заказа<br />и особого события</h2><p>Заказ готовой еды, повтор покупки и заявка на торт начинаются с разных действий и не мешают друг другу.</p></div><div className="fit-grid"><article className="daily"><span>Каждый день</span><h3>Готовая еда</h3><p>Место → категория → корзина → время.</p><div className="mini-path"><PackageCheck /><ArrowRight /><Store /><ArrowRight /><Clock3 /></div></article><article className="habit"><span>Снова</span><h3>Избранное и повтор</h3><p>Товары можно сохранить, а прошлый заказ — проверить и повторить.</p><div className="mini-path"><Heart /><ArrowRight /><RefreshCw /></div></article><article className="event"><span>К событию</span><h3>Индивидуальный торт</h3><p>Заявка сохраняет повод, оформление, дату и телефон.</p><div className="mini-path"><CakeSlice /><ArrowRight /><Check /></div></article></div></section>
  )
}
...
function Metrics() {
  const metrics = ['Конверсия из оформления в принятый заказ', 'Время и количество шагов до заказа', 'Частота ошибок оплаты и доля сохранённых корзин', 'Доля завершённых повторных заказов', 'Использование самовывоза и карты лояльности', 'Доля сохранённых заявок на индивидуальный торт']
  return (
