...
function Cover() {
  return (
    <section className="cover-layout">
      ...
      <div className="cover-visual"><div className="orange-orbit" /><Phone src="assets/case/home.jpg" alt="Главный экран концепции" /><div className="float-chip card-chip"><WalletCards /> Бонусы после входа</div><div className="float-chip pickup-chip"><Store /> Точка до меню</div></div>
    </section>
  )
}
...
function LoyaltyScenario() {
  return (
    <section className="scenario-layout reverse"><div className="phones-pair"><Phone src="assets/case/login.jpg" alt="Вход по коду" /><Phone src="assets/case/loyalty.jpg" alt="Виртуальная карта с QR-кодом" tone="front" /></div><div className="slide-copy"><SectionLabel number="06">Ключевой сценарий № 2</SectionLabel><h2>Бонусы открываются<br />после входа</h2><p>Гостевой старт не раскрывает персональные данные. После входа пользователь получает баланс и демонстрационный QR-код виртуальной карты.</p><ul className="check-list"><li><Check /> код из СМС вместо постоянного пароля</li><li><Check /> баланс и QR скрыты до входа</li><li><Check /> официальные правила открываются по ссылке</li></ul><LinkButton href={proto('/loyalty')}>Открыть бонусы</LinkButton></div></section>
  )
}

function CakeScenario() {
  return (
    <section className="scenario-layout"><div className="slide-copy"><SectionLabel number="07">Ключевой сценарий № 3</SectionLabel><h2>Торт — честный<br />бриф на расчёт</h2><p>Пользователь указывает повод, гостей, визуальный стиль, дату и телефон. После сохранения сотрудник связывается для уточнения начинки, оформления и стоимости.</p><div className="cake-steps"><span>1<small>Повод</small></span><span>2<small>Гости</small></span><span>3<small>Стиль</small></span><span>4<small>Контакт</small></span><span>5<small>Проверка</small></span></div><LinkButton href={proto('/cake?step=occasion')}>Оставить бриф</LinkButton></div><div className="cake-showcase"><Phone src="assets/case/cake.jpg" alt="Сценарий брифа на индивидуальный торт" /><div className="cake-card"><CakeSlice /><span>Сотрудник свяжется для расчёта</span></div></div></section>
  )
}

function BusinessFit() {
  return (
    <section className="standard-layout fit-slide"><div className="slide-copy"><SectionLabel number="08">Связь с бизнесом</SectionLabel><h2>Один интерфейс<br />для разных частот</h2><p>Регулярная готовая еда и редкий торт не конкурируют на одном экране — у каждого сценария свой темп.</p></div><div className="fit-grid"><article className="daily"><span>Каждый день</span><h3>Быстрая готовая еда</h3><p>Место → категория → корзина → время.</p><div className="mini-path"><PackageCheck /><ArrowRight /><Store /><ArrowRight /><Clock3 /></div></article><article className="habit"><span>Регулярно</span><h3>Избранное и повтор</h3><p>Любимые позиции сохраняются; прошлый заказ проверяется перед добавлением.</p><div className="mini-path"><Heart /><ArrowRight /><RefreshCw /></div></article><article className="event"><span>К событию</span><h3>Бриф на торт</h3><p>Повод, стиль, дата и контакт сохраняются для последующего расчёта.</p><div className="mini-path"><CakeSlice /><ArrowRight /><Check /></div></article></div></section>
  )
}

function Pilot() {
  return (
    <section className="pilot-layout"><div className="slide-copy"><SectionLabel number="09">Предлагаемый пилот</SectionLabel><h2>Сначала три пути,<br />которые можно измерить</h2><p>Объём и интеграции определяются после проверки действующих систем. Концепция не предполагает заранее наличие конкретного API.</p></div><div className="pilot-timeline"><article><b>01</b><div><strong>Бонусы и вход</strong><span>Проверить скорость доступа и долю успешных открытий у кассы.</span></div></article><article><b>02</b><div><strong>Заказ готовой еды</strong><span>Сравнить путь с выбором точки до корзины.</span></div></article><article><b>03</b><div><strong>Ошибка и повтор</strong><span>Проверить долю восстановленных корзин и завершённых повторных заказов.</span></div></article><article className="pilot-result"><Check /><div><strong>Решение по следующему этапу</strong><span>На основании сравнимых данных пилота; целевые проценты заранее не задаются.</span></div></article></div></section>
  )
}
