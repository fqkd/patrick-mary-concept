...
const cakeStyles = [
  { name: 'Светлое и воздушное', image: products.find((item) => item.id === 'sour-cream-cake')?.image ?? 'assets/food/cakes.jpg' },
  { name: 'Ягодный акцент', image: products.find((item) => item.id === 'royal-velvet-cake')?.image ?? 'assets/food/cakes.jpg' },
  { name: 'Лаконичная надпись', image: products.find((item) => item.id === 'school-sweet-lesson')?.image ?? 'assets/food/cakes.jpg' },
]
...
export function App() {
  ...
  const chooseLocation = (mode: ServiceMode, location: string) => {
    ...
    setState((current) => ({
      ...
      basketNotice: changed && current.cart.length ? 'Корзина проверена для нового адреса' : '',
    }))
  ...
  const resolveSwitch = (replace: boolean) => {
    setState((current) => {
      ...
      return {
        ...
        slot: '',
        pendingSwitch: null,
        basketNotice: replace ? 'Недоступные позиции заменены для нового адреса' : 'Недоступные позиции удалены для нового адреса',
      }
    })
  ...
  else if (route.path === '/repeat') content = <RepeatScreen currentCart={state.cart} onUse={(cart) => { setState((current) => ({ ...current, mode: 'pickup', location: 'ул. Красная, 155', cart, slot: '', basketNotice: 'Корзина собрана по актуальным ценам' })); navigate('/cart') }} />
  ...
  return (
    <div className="device-stage">
      ...
      <aside className="desktop-note">
        ...
        <p>Выберите получение, соберите корзину и пройдите вход. В демо первый платёж показывает восстановление после ошибки, повторный — подтверждает заказ.</p>
        <div className="desktop-tip"><Smartphone /><span><strong>Как пройти сценарий</strong>Для входа и повторной оплаты подойдёт любой четырёхзначный код.</span></div>
...
function HomeScreen({ state, onMode }: { state: DemoState; onMode: (mode: ServiceMode) => void }) {
  return (
    <div className="screen with-nav home-screen">
      ...
      <section className="welcome-card"><span className="eyebrow">Готовая еда на каждый день</span><h1>Что приготовим<br />для вашего дня?</h1><p>Сначала проверим точку и время — затем покажем доступное.</p><div className="welcome-actions"><button className="primary light" onClick={() => onMode('delivery')}><PackageCheck /> Доставка</button><button className="primary ghost-light" onClick={() => onMode('pickup')}><Store /> Самовывоз</button></div></section>
      <section className="quick-grid">
        <button className={`loyalty-tile${state.authenticated ? '' : ' guest'}`} onClick={() => navigate(state.authenticated ? '/loyalty' : '/login?step=phone&next=loyalty')}><span><WalletCards /> Бонусы</span>{state.authenticated ? <><strong>1 240</strong><small>баллов · QR-код доступен</small></> : <><strong>Войти</strong><small>Получить виртуальную карту</small></>}</button>
        <button className="cake-tile" onClick={() => navigate('/cake?step=occasion')}><span className="cake-tile-kicker">Индивидуальный торт</span><strong>Оставить бриф</strong><small>Повод, стиль, дата и контакт</small><span className="cake-tile-arrow"><ArrowRight /></span></button>
      </section>
      {state.authenticated ? <section className="section-block"><div className="section-title"><div><span className="eyebrow">В прошлый раз</span><h2>Повторить к ужину</h2></div><button onClick={() => navigate('/repeat')}>Проверить</button></div><button className="repeat-card" onClick={() => navigate('/repeat')}><div className="repeat-images"><img src={products.find((product) => product.id === 'syrniki')?.image} alt="Сырник творожный" /><img src={products.find((product) => product.id === 'bakery')?.image} alt="Киш с рыбой" /></div><div><strong>4 единицы · тогда {money(total(previousOrder))}</strong><span>Проверим цену и наличие</span></div><ChevronRight /></button></section> : <section className="section-block guest-section"><UserRound /><div><strong>Заказы и адреса появятся после входа</strong><span>История, повтор и персональные данные скрыты в гостевом режиме.</span></div><button onClick={() => navigate('/login?step=phone&next=orders')}>Войти</button></section>}
      <section className="section-block"><div className="section-title"><div><span className="eyebrow">Сегодня</span><h2>Можно забрать по пути</h2></div></div><div className="editorial-card"><img src="assets/food/salads.jpg" alt="Салат с курицей и овощами" /><div><span>На сегодня</span><strong>Ужин без лишней готовки</strong><button onClick={() => onMode('pickup')}>Выбрать кулинарию <ArrowRight /></button></div></div></section>
    </div>
  )
}

function ModeScreen({ onChoose }: { onChoose: (mode: ServiceMode) => void }) {
  return <div className="screen paper-screen"><Header title="Как получите заказ?" /><div className="screen-body"><span className="eyebrow">До корзины</span><h1>Сначала место<br />и способ получения</h1><p className="lead">Так каталог сможет показать ассортимент выбранной точки и доступное время.</p><button className="mode-card delivery" onClick={() => onChoose('delivery')}><span className="mode-icon"><PackageCheck /></span><span><strong>Доставка</strong><small>Укажем адрес и проверим доступность</small></span><ChevronRight /></button><button className="mode-card pickup" onClick={() => onChoose('pickup')}><span className="mode-icon"><Store /></span><span><strong>Самовывоз</strong><small>Выберем конкретную кулинарию</small></span><ChevronRight /></button></div></div>
}
...
function BasketCheckScreen({ pending, onResolve }: { pending: PendingSwitch | null; onResolve: (replace: boolean) => void }) {
  if (!pending) return <div className="screen result-screen"><h1>Корзина уже проверена</h1><button className="primary" onClick={() => navigate('/catalog?category=all')}>Вернуться в меню</button></div>
  return <div className="screen paper-screen"><Header title="Проверка корзины" back="/mode" /><div className="screen-body"><span className="eyebrow">Новый адрес</span><h1>Состав нужно<br />обновить</h1><p className="lead">Для {pending.location} недоступны:</p><div className="compare-list">{pending.unavailable.map((line) => <div className="unavailable" key={line.id}><span><X /><strong>{line.name} · {line.quantity} шт.</strong></span><span>Можно заменить на мясное ассорти или удалить</span></div>)}</div><div className="why-note"><RefreshCw /><p><strong>Остальные позиции сохранятся.</strong> Сумма пересчитается до продолжения.</p></div></div><div className="bottom-cta basket-actions"><button className="primary" onClick={() => onResolve(true)}>Заменить недоступные</button><button className="secondary" onClick={() => onResolve(false)}>Удалить и продолжить</button></div></div>
}
...
function FavoritesScreen({ state, onAdd, onToggleFavorite }: { state: DemoState; onAdd: (product: Product) => void; onToggleFavorite: (id: string) => void }) {
  ...
  return <div className="screen with-nav paper-screen favorites-screen"><Header title="Избранное" /><div className="favorites-body"><div className="menu-heading"><div><span className="eyebrow">Сохранённое</span><h1>Избранное</h1><small>{favoriteProducts.length} {positionWord(favoriteProducts.length)}</small></div></div>{favoriteProducts.length === 0 ? <div className="empty-state tall"><Heart /><h2>Здесь пока пусто</h2><p>Добавляйте блюда из меню.</p><button className="primary" onClick={() => navigate('/catalog?category=all')}>Открыть меню</button></div> : <div className="product-grid">{favoriteProducts.map((product) => <ProductCard key={product.id} product={product} favorite onToggleFavorite={() => onToggleFavorite(product.id)} onAdd={() => onAdd(product)} />)}</div>}</div></div>
}

function CartScreen({ state, onChange }: { state: DemoState; onChange: (id: string, delta: number) => void }) {
  return <div className="screen paper-screen cart-screen"><Header title="Корзина" back="/catalog?category=all" /><div className="screen-body">{state.cart.length === 0 ? <div className="empty-state tall"><ShoppingBag /><h1>Корзина ждёт</h1><p>Выберите получение и добавьте блюда.</p><button className="primary" onClick={() => navigate('/mode')}>Начать заказ</button></div> : <><button className="order-context" onClick={() => navigate('/mode')}><span><small>{state.mode === 'delivery' ? 'Доставка' : 'Самовывоз'}</small><strong>{state.location}</strong></span><ChevronRight /></button><div className="cart-lines">{state.cart.map((line) => <div className="cart-line" key={line.id}><img src={products.find((item) => item.id === line.id)?.image ?? 'assets/food/bakery.jpg'} alt="" /><span><strong>{line.name}</strong><small>{money(line.price)}</small></span><div className="stepper"><button aria-label={`Уменьшить ${line.name}`} onClick={() => onChange(line.id, -1)}><Minus /></button><b>{line.quantity}</b><button aria-label={`Увеличить ${line.name}`} onClick={() => onChange(line.id, 1)}><Plus /></button></div></div>)}</div><div className="cart-note"><Clock3 /><span><strong>Следующий шаг — время</strong><small>Интервалы зависят от выбранной точки.</small></span></div><div className="summary"><span>Позиции</span><b>{money(total(state.cart))}</b><span>Время</span><b>{state.slot || 'выберите далее'}</b></div></>}</div>{state.cart.length > 0 && <div className="bottom-cta"><button className="primary" onClick={() => navigate('/time')}>Выбрать время <ArrowRight /></button></div>}</div>
}
...
function RepeatScreen({ currentCart, onUse }: { currentCart: CartLine[]; onUse: (cart: CartLine[]) => void }) {
  ...
  return <div className="screen paper-screen"><Header title="Повтор заказа" /><div className="screen-body"><span className="eyebrow">Заказ от 28 июля</span><h1>Проверили цену<br />и наличие</h1><p className="lead">Исходный заказ: 4 единицы. Недоступная позиция не войдёт в новую корзину.</p><div className="compare-list">{reconciled.map((line) => <div key={line.id} className={!line.available ? 'unavailable' : ''}><span>{line.available ? <Check /> : <X />}<strong>{line.name} · {line.quantity} шт.</strong></span><span>{line.available ? `Тогда ${money(previousOrder.find((item) => item.id === line.id)!.price * line.quantity)} · сейчас ${money(line.price * line.quantity)}` : `Тогда ${money(line.price * line.quantity)} · сейчас недоступно`}</span></div>)}</div><div className="price-compare"><span><small>Тогда</small><strong>{money(total(previousOrder))}</strong></span><ArrowRight /><span><small>Сейчас</small><strong>{money(total(available))}</strong></span></div>{currentCart.length > 0 && <div className="why-note"><RefreshCw /><p><strong>Текущая корзина будет заменена.</strong> В неё попадут 3 доступные единицы.</p></div>}</div><div className="bottom-cta"><button className="primary" onClick={() => onUse(available)}>{currentCart.length > 0 ? 'Заменить корзину' : 'Собрать корзину'} · {money(total(available))} <ArrowRight /></button></div></div>
}
...
function CakeScreen({ step, authenticated, draft, onDraft, onSave }: { step: string | null; authenticated: boolean; draft: CakeDraft; onDraft: (patch: Partial<CakeDraft>) => void; onSave: () => void }) {
  ...
  return <div className="screen cake-screen"><Header title="Индивидуальный торт" /><div className="cake-visual"><img src={style.image} alt={`Пример: ${style.name}`} /><span>Шаг {index + 1} из {cakeStepIds.length}</span></div><div className="cake-body">
    ...
    {currentStep === 'style' && <><span className="eyebrow">Референс</span><h1>Какой стиль ближе?</h1><div className="style-grid">{cakeStyles.map((value) => <button className={draft.design === value.name ? 'selected' : ''} key={value.name} onClick={() => onDraft({ design: value.name })}><img src={value.image} alt="" /><span>{value.name}</span>{draft.design === value.name && <Check />}</button>)}</div><p className="fine-print">Фото передаёт направление оформления, а не точную копию будущего торта.</p></>}
    {currentStep === 'details' && <><span className="eyebrow">Дата и контакт</span><h1>Когда связаться?</h1><label className="form-field"><span><CalendarDays />Желаемая дата</span><input type="date" min="2026-08-14" value={draft.date} onChange={(event) => onDraft({ date: event.target.value })} aria-label="Желаемая дата" /></label><label className="form-field"><span><Phone />Телефон</span><input value={draft.phone} onChange={(event) => onDraft({ phone: event.target.value })} placeholder="+7 900 000 00 00" aria-label="Телефон для связи" /></label><label className="form-field textarea"><span><MessageSquare />Комментарий</span><textarea value={draft.comment} onChange={(event) => onDraft({ comment: event.target.value })} placeholder="Начинка, надпись или пожелания" aria-label="Комментарий к заявке" /></label>{authenticated && <small className="helper">Использован номер из профиля — его можно изменить.</small>}</>}
    {currentStep === 'review' && <><span className="eyebrow">Проверка</span><h1>Ваш бриф</h1><div className="cake-preview"><img src={style.image} alt={draft.design} /><span>{draft.design}</span></div><div className="receipt"><span><small>Повод</small><strong>{draft.occasion}</strong></span><span><small>Гости</small><strong>{draft.guests}</strong></span><span><small>Дата</small><strong>{draft.date}</strong></span><span><small>Телефон</small><strong>{draft.phone}</strong></span>{draft.comment && <span><small>Комментарий</small><strong>{draft.comment}</strong></span>}</div><p className="fine-print">После сохранения сотрудник свяжется с вами, чтобы уточнить начинку, оформление и стоимость.</p></>}
...
function CakeConfirmScreen({ request }: { request: CakeRequest | null }) {
  ...
  return <div className="screen result-screen success-result"><div className="result-icon"><CakeSlice /></div><span className="eyebrow">Заявка сохранена</span><h1>Бриф получен</h1><p>Сотрудник свяжется по указанному номеру, чтобы уточнить детали и рассчитать стоимость.</p><div className="receipt"><span><small>Повод</small><strong>{request.occasion}</strong></span><span><small>Гости</small><strong>{request.guests}</strong></span><span><small>Стиль</small><strong>{request.design}</strong></span><span><small>Дата</small><strong>{request.date}</strong></span><span><small>Телефон</small><strong>{request.phone}</strong></span></div><button className="primary" onClick={() => navigate('/')}>На главную</button><button className="text-button" onClick={() => navigate('/cake?step=occasion')}>Изменить заявку</button></div>
}
...
function DemoQr() {
  ...
  return <div className="demo-qr" aria-label="Демонстрационный QR-код карты"><svg viewBox="0 0 21 21" role="img">{cells.map((dark, index) => dark && <rect key={index} x={index % 21} y={Math.floor(index / 21)} width="1" height="1" />)}</svg><span>ДЕМО</span></div>
}

function LoyaltyScreen({ authenticated }: { authenticated: boolean }) {
  if (!authenticated) return <div className="screen with-nav paper-screen"><Header title="Бонусы" /><GuestGate icon={<WalletCards />} title="Войдите, чтобы открыть карту" text="После регистрации здесь появятся баланс и демонстрационный QR-код виртуальной карты." next="loyalty" /></div>
  return <div className="screen with-nav loyalty-screen"><Header title="Бонусы" /><div className="loyalty-card"><span className="wordmark">ПАТРИК <i>&</i> МАРИ</span><DemoQr /><div><strong>1 240</strong><small>баллов</small></div></div><div className="offline-banner"><Smartphone /><span><strong>QR-код доступен без лишних шагов</strong><small>Покажите демонстрационный код сотруднику до оплаты.</small></span></div><section className="section-block"><div className="section-title"><div><span className="eyebrow">Программа лояльности</span><h2>Начисление и списание</h2></div></div><div className="info-list"><span><WalletCards /><div><strong>Покажите QR-код на кассе</strong><small>Баланс обновится после покупки.</small></div></span><a href="https://patrickmary.ru/bonusy" target="_blank" rel="noreferrer"><ExternalLink /><div><strong>Правила программы</strong><small>Открыть официальные условия.</small></div></a></div></section></div>
}
...
function LoginScreen({ step, next, phone, onPhone, onAuthenticated }: { step: string | null; next: string | null; phone: string; onPhone: (phone: string) => void; onAuthenticated: () => void }) {
  ...
  return <div className="screen login-screen"><Header title="Вход" /><div className="login-illustration"><Smartphone /></div><div className="screen-body"><span className="eyebrow">Без постоянного пароля</span><h1>{codeStep ? 'Введите код из СМС' : 'Номер телефона — и вы внутри'}</h1><p className="lead">{codeStep ? `Код отправлен на ${phone}.` : 'Укажите номер, чтобы получить код для входа.'}</p>{!codeStep ? <><label className="phone-input"><span>+7</span><input value={phone.replace(/^\+7\s*/, '')} onChange={(event) => onPhone(`+7 ${event.target.value}`)} aria-label="Номер телефона" /></label><button className="primary" onClick={() => navigate(`/login?step=code&next=${nextValue}`)}>Получить код</button></> : <><label className="code-input"><input value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 4))} inputMode="numeric" placeholder="0000" aria-label="Код из СМС" /></label><button className="primary" disabled={code.length !== 4} onClick={onAuthenticated}>Войти</button><button className="text-button" onClick={() => navigate(`/login?step=phone&next=${nextValue}`)}>Изменить номер</button></>}</div></div>
}
