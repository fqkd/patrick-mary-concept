  ...
  return (
    <div className="screen with-nav home-screen">
      ...
      <section className="quick-grid">
        ...
        <button className="cake-tile" onClick={() => navigate('/cake?step=occasion')}><img className="cake-tile-image" src={products.find((product) => product.id === 'sour-cream-cake')?.image} alt="" /><span className="cake-tile-kicker">Торт на заказ</span><strong>Оставить заявку</strong><small>Повод, оформление, дата и телефон</small><span className="cake-tile-arrow"><ArrowRight /></span></button>
      </section>
...
function LoyaltyScreen({ authenticated }: { authenticated: boolean }) {
  ...
  return <div className="screen with-nav loyalty-screen"><Header title="Бонусы" /><div className="loyalty-card"><div className="loyalty-card-copy"><span className="wordmark">ПАТРИК <i>&</i> МАРИ</span><div className="loyalty-balance"><strong>1 240</strong><small>баллов</small></div></div><LoyaltyQr /></div><section className="section-block"><div className="section-title"><div><span className="eyebrow">Программа лояльности</span><h2>Как использовать карту</h2></div></div><div className="info-list"><span><WalletCards /><div><strong>Покажите QR-код на кассе</strong><small>Условия начисления и списания указаны в правилах программы.</small></div></span><a href="https://patrickmary.ru/bonusy" target="_blank" rel="noreferrer"><ExternalLink /><div><strong>Правила программы</strong><small>Открыть на сайте «Патрик & Мари».</small></div></a></div></section></div>
}
