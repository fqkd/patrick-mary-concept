...
await scenario('URL — источник истины и reload', 390, async (page) => {
  ...
  await page.getByText('Кулинарии', { exact: true }).waitFor()
})
...
async function fullJourney(page) {
  ...
  await assertHash(page, '/location?mode=pickup')
  await page.locator('.pm-map-pin').first().waitFor()
  if (await page.locator('.pm-map-pin').count() !== 21) throw new Error('на карте не 21 маркер')
  if (await page.locator('.location-list button').count() !== 5) throw new Error('по умолчанию показано не пять точек')
  const pickupSearch = page.getByRole('textbox', { name: 'Поиск кулинарии' })
  await pickupSearch.fill('Карасунский')
  if (await page.locator('.location-list button').count() !== 1) throw new Error('поиск по округу не работает')
  await page.getByText('ул. Сормовская, 108/2', { exact: true }).waitFor()
  await pickupSearch.fill('')
  await page.getByRole('button', { name: /Показать все/ }).click()
  if (await page.locator('.location-list button').count() !== 21) throw new Error('полный список не раскрывается внутри приложения')
  await page.locator('.pm-map-pin[title="ул. Сормовская, 108/2"]').click()
  const selectedRow = page.locator('.location-list button.active')
  if (!await selectedRow.getByText('ул. Сормовская, 108/2').count()) throw new Error('маркер не синхронизирован со строкой')
  await page.getByRole('button', { name: 'Выбрать эту кулинарию' }).click()
  await page.getByText('Самовывоз · ул. Сормовская, 108/2').waitFor()
